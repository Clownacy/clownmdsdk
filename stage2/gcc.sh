...
patch -sNd gcc-15.1.0 -p1 < FixMinGWClang.patch

mkdir -p build-gcc
cd build-gcc
../gcc-15.1.0/configure --target=$TARGET --prefix="$PREFIX" --disable-nls --enable-languages=c,c++ --without-headers --disable-multilib --with-cpu=68000 --disable-hosted-libstdcxx
